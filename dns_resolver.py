...
def remove_ns_records_and_reset_qr(response):
    """
    Removes NS records from a DNS response and resets the QR bit to 0.
    
    Args:
        response (bytes): The DNS response message.
    
    Returns:
        bytes: The modified DNS message.
    """
...
while True:
    ...
    # TLD SERVER
    print("\n\nTLD SERVER:")
    tld_query = remove_ns_records_and_reset_qr(root_response) #We should extract the ns server from the Root ns records and then send the query to the TLD server, however since this project is a local dns system with only 1 Ip Address we will use a constant value (127.0.0.1) 
